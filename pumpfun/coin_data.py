...
from pumpfun.config import client
...
async def get_virtual_reserves(bonding_curve):
...
async def sol_for_tokens(sol_spent, sol_reserves, token_reserves):
...
async def tokens_for_sol(tokens_to_sell, sol_reserves, token_reserves):
